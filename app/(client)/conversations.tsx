  ...
  const loadConversations = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        id,
        professional_id,
        professional:profiles!professional_id(name, avatar_url)
      `)
    ...
    const enriched = await Promise.all(
      data.map(async (conv) => {
        const prof = conv.professional as any
        ...
        return {
          ...
          professional_avatar: prof?.avatar_url ?? null,
      ...
      {conversations.length === 0 ? (
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-gray-500 text-center text-base leading-6">
            Você ainda não tem conversas. Inicie contato com um profissional.
          </Text>
        </View>
      ) : (
        <FlatList
